    ...
    if (item.count > 0) {
      return (
        <div className="order-burger" key={index}>
          ...
          <button onClick={() => {document.querySelector('.burger-cart').style.zIndex = 1; setFormdata({...formdata, nomi:item.name , narx: item.price * item.count,son: item.count})}}>Yetkazib berish</button>
...
<form onSubmit={handleSubmit}>
          ...
          <button type="submit" onClick={() => document.querySelector('.burger-cart').style.zIndex = -1}>Yuborish</button>
